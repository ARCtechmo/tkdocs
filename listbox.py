...
# listbox
# displays a list of single-line text items allowing users to browse through the list
# Note: tkinter does not allow you to directly link regular Python variables to a list
choices  = ['apple', 'orange','banana']
choicevar = StringVar(value=choices)
lbox_Var = Listbox(root, height=10,listvariable=choicevar)
choices.append("peace")
choicevar.set(choices)

# example
# select a gift to send to a list of countries
# ** Use parallel lists 
# - the first list is for display to the user only
# - the second list is associated with some other underlying value 
# - the user is not intended to see the second list
...
cnames = StringVar(value=countrynames) # Tk string variable NOT a normal Python var
...
c.grid(column=0,row=0, sticky=(N,S,E,S))
